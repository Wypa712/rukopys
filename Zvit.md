...
- Added best-checkpoint saving to the OCR notebook. During training it now writes `crnn_rukopys_best.pt` whenever validation CER improves, stores `best_epoch` and `best_val_cer`, saves the final checkpoint separately as `crnn_rukopys.pt`, and loads the best checkpoint for the demo prediction when available.
- Updated `RUKOPYS_TRACKER.md`: best-checkpointing is now marked done, and the recommended next implementation task is YOLO-style detector dataset export from page annotations.
- Full-dataset preprocessing appeared to hang around `normalize_text`, but the function itself has no unbounded loop. The notebook now uses `tqdm` progress bars in `build_ocr_samples`, uses `ds["source"]` for stratification instead of indexing every full row, raises PIL's image-size limit for large scans, and uses a translation table for text normalization.
- Full OCR training reportedly reached validation error/CER around `0.01`, which is strong enough to treat the line-level OCR baseline as usable. Before relying on it, confirm the run was from a clean Kaggle restart and that validation still uses the page-level split; after that, the next bottleneck is page-level region detection and submission formatting.
- Added `export_detector_dataset.py` for YOLO-style detector data export. It defaults to `--classes handwritten`, writes `images/train`, `images/val`, `labels/train`, `labels/val`, and `data.yaml`, and includes helper functions for bbox conversion/class filtering.
- Added `tests/test_export_detector_dataset.py`; local unit tests pass with `python -m unittest tests.test_export_detector_dataset`. The export itself still needs to be run in Kaggle or another environment with the `datasets` package and RUKOPYS access.
